...
public class CompendiumServiceDataSourceFactory
		extends BundleClassLoaderDataSourceFactory {

	private static final Log LOG = LogFactory
			.getLog(CompendiumServiceDataSourceFactory.class);
	 ...
	 */
	public static final String FLYWAY_DRIVER_NAME_PROPERTY = Flyway.FLYWAY_PROPERTY_PREFIX
			+ "driverName";
	 ...
	 */
	public static final String FLYWAY_DRIVER_VERSION_PROPERTY = Flyway.FLYWAY_PROPERTY_PREFIX
			+ "driverVersion";

	private BundleContext context;

	public CompendiumServiceDataSourceFactory(BundleContext context) {
	...
	private Driver loadCompendiumDriver(Properties config)
			throws ConfigurationException {
		Properties properties = getJdbcProperties(config);
		ServiceReference serviceReference = lookupDataSourceFactory(
				config);
		try {
			DataSourceFactory dataSourceFactory = (DataSourceFactory) context
					.getService(serviceReference);
			return dataSourceFactory == null ? null
					: dataSourceFactory.createDriver(properties);
	 ...
	 * name.
	 */
	private ServiceReference lookupDataSourceFactory(Properties config)
			throws ConfigurationException {

		OsgiJdbcServiceFilter filter = new OsgiJdbcServiceFilter(config,
				FLYWAY_DRIVER_PROPERTY, FLYWAY_DRIVER_NAME_PROPERTY,
				FLYWAY_DRIVER_VERSION_PROPERTY);

		try {
			ServiceReference[] serviceReferences = context.getServiceReferences(
					DataSourceFactory.class.getName(), filter.toString());

			if (serviceReferences == null || serviceReferences.length == 0) {
				throw new ConfigurationException(FLYWAY_DRIVER_PROPERTY,
						"No OSGI JDBC Compendium DataSourceFactory found for filter '"
								+ filter + "'");
			}

			if (serviceReferences.length > 1) {
				LOG.warn(
						"Found multiple DataSourceFactory services for filter '"
								+ filter
			...
			throw new ConfigurationException(FLYWAY_DRIVER_PROPERTY,
					"Could not lookup the OSGI Compendium JDBC DataSourceFactory",
					e);
		}
	}
